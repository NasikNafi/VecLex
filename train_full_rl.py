...
from veclex_metric import compute_veclex_with_unigram_lex, compute_veclex_with_bigram_lex
...
def train(args):
    ...
    train_params = configure_training(
        ...
        args.gamma, args.reward, args.stop, args.stop_reward
    )
    train_batcher, val_batcher = build_batchers(args.batch)
    # TODO different reward
    reward_fn = compute_veclex_with_bigram_lex
    stop_reward_fn = compute_veclex_with_unigram_lex
...
if __name__ == '__main__':
    ...
    # training options
    parser.add_argument('--reward', action='store', default='veclex-bigram',
                        help='reward function for RL')
    parser.add_argument('--stop_reward', action='store', default='veclex-unigram',
                        help='reward function for RL')
    ...
    parser.add_argument('--stop', type=float, action='store', default=1.0,
                        help='stop coefficient for veclex-unigram')
