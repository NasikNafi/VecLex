    ...
    with torch.no_grad():
        for art_batch, abs_batch in loader:
            ...
            for (j, n), abs_sents in zip(ext_inds, abs_batch):
                ...
                avg_reward += compute_veclex(list(concat(summs)),
...
def a2c_train_step(agent, abstractor, loader, opt, grad_fn,
                   gamma=0.99, reward_fn=compute_veclex_with_bigram_lex,
                   stop_reward_fn=compute_veclex_with_unigram_lex, stop_coeff=1.0):
    opt.zero_grad()
    indices = []
    probs = []
    baselines = []
...
class A2CPipeline(BasicPipeline):
    ...
    def checkpoint(self, *args, **kwargs):
        # explicitly use inherited function
