"""produce the dataset with (pseudo) extraction label"""
import os
import sys
...
def get_extract_label(art_sents, abs_sents):
    ...
    for abst in abs_sents:
        sim_scores = list(map(compute_rouge_l(reference=abst, mode='r'),
                          art_sents))
        ext = max(indices, key=lambda i: sim_scores[i])
        indices.remove(ext)
        extracted.append(ext)
        scores.append(sim_scores[ext])
        if not indices:
            break
